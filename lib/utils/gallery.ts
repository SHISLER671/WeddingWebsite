...
export async function uploadGalleryFile(file: File, caption = "", uploaderName: string): Promise<UploadResult> {
  try {
  ...
  try {
    ...
    const itemsWithUrls = await Promise.all(
      (data || []).map(async (item) => {
        ...
        // Check if file_path is a data URL (base64)
        if (!item.file_path.startsWith("data:")) {
          // It's a storage path, get the public URL
          const { data: urlData } = supabase.storage.from("wedding-gallery").getPublicUrl(item.file_path)

          fileUrl = urlData.publicUrl
        }

        // Determine file type
        const isDataUrl = item.file_path.startsWith("data:")
        let isVideo = false

        if (isDataUrl) {
          // Check MIME type in data URL
          isVideo = item.file_path.startsWith("data:video/")
        } else {
          // Check file extension
          const fileExt = item.file_path.split(".").pop()?.toLowerCase()
          isVideo = ["mp4", "mov", "avi", "webm", "mkv"].includes(fileExt || "")
        }
...
export function getOptimizedImageUrl(filePath: string, width = 400): string {
  const supabase = createClient()
...
export async function deleteGalleryItem(itemId: string): Promise<boolean> {
  try {
    ...
    // Delete from storage
    const { error: storageError } = await supabase.storage.from("wedding-gallery").remove([item.file_path])
    ...
    const { error: dbError } = await supabase.from("gallery_items").delete().eq("id", itemId)
