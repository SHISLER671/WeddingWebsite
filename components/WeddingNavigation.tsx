  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 bg-jewel-burgundy/90 backdrop-blur-sm text-warm-white p-3 rounded-full shadow-lg hover:bg-jewel-crimson transition-all duration-300 hover:scale-105 border-2 border-jewel-gold/30"
      ...
      <div className={`fixed inset-0 z-40 transition-all duration-300 ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        ...
        {/* Menu Panel */}
        <div className={`absolute top-0 left-0 h-full w-80 max-w-[85vw] bg-jewel-burgundy/95 backdrop-blur-md border-r-2 border-jewel-gold/50 transform transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
